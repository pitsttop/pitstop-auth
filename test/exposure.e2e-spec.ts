...
describe('Exposure tests (sensitive data)', () => {
  beforeAll(async () => {
    ...
    // ensure migrations/schema applied (best-effort)
  ...
  it('signup response must not expose password', async () => {
    const newUser = { name: 'Expose', email: 'expose@teste.com', password: 'Segredo123!' };
    const res = await request(app).post('/auth/signup').send(newUser);
    expect(res.status).toBe(201);
    // Response should include user metadata but not password
    expect(res.body).toHaveProperty('user');
    expect(res.body.user).not.toHaveProperty('password');
    // top-level should not include password either
    expect(res.body).not.toHaveProperty('password');

    // Verify DB has stored a hashed password (not equal to plaintext)
  ...
  it('login response must not expose password', async () => {
    const newUser = { name: 'LoginExpose', email: 'login-expose@teste.com', password: 'Segredo123!' };
    await request(app).post('/auth/signup').send(newUser);

    const res = await request(app).post('/auth/login').send({ email: newUser.email, password: newUser.password });
  ...
  it('error responses should not echo sensitive fields', async () => {
    const usr = { name: 'Dup2', email: 'dup2@teste.com', password: 'Segredo123!' };
    const r1 = await request(app).post('/auth/signup').send(usr);
    expect(r1.status).toBe(201);

    // Attempt duplicate signup — error should not include the password
